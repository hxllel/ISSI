...
export function Alumno() {
  ...
  const handleDescargarComprobante = () => {
    try {
      ...
      const fotoUrl =
        alumno && alumno.id ? `http://localhost:4000/Alumno/Foto/${alumno.id}` : "";
      ...
      const cabecera = `
        ...
        <div class="header-container">
          ${
            fotoUrl
              ? `<img src="${fotoUrl}" alt="Foto alumno" class="foto-alumno" onerror="this.style.display='none'" />`
              : ""
          }
          <div class="meta-info">
            <div class="meta">
              ${
                alumno
                  ? `<div><strong>Alumno:</strong> ${alumno.nombre || ""} ${
                      alumno.ape_paterno || ""
                    } ${alumno.ape_materno || ""}</div>`
                  : ""
              }
              ${
                alumno
      ...
      if (!w) {
        alert(
          "No se pudo abrir la ventana de impresión. Permite ventanas emergentes para continuar."
        );
        return;
      }
      w.document.open();
